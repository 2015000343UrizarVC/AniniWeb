...
async function cargarDatos() {
  ...
  console.log("✅ Tabla actualizada con", snapshot.size, "registros ordenados por ID");
}
...
formulario.addEventListener("submit", async (e) => {
  ...
  if (existente.exists()) {
    return alert("❌ Ya existe un registro con ese ID. Usa otro.");
  }
  ...
  const data = {
    ...
    inventario: parseInt(document.getElementById("inventario").value) || 0,
...
// 🧹 Normalizar documentos antiguos para que todos tengan campo 'id'
window.normalizarIDs = async function () {
  ...
  snapshot.forEach(docSnap => {
    ...
    if (!data.id) {
      const ref = doc(dbRef, docSnap.id);
      promesas.push(updateDoc(ref, { id: docSnap.id }));
    }
  });

  await Promise.all(promesas);
  console.log("✅ Todos los documentos ahora tienen campo 'id'");
  alert("🧼 Normalización completada: todos los registros tienen ID");
  cargarDatos(); // Actualiza la tabla después de normalizar
};
...
window.subirDatosDesdeMySQL = async function () {
  try {
    const ids = new Set();
...
window.eliminarTodosLosDocumentos = async function () {
  const snapshot = await getDocs(dbRef);
  const promesas = [];

  snapshot.forEach(docSnap => {
    const ref = doc(dbRef, docSnap.id);
    promesas.push(deleteDoc(ref));
  });

  await Promise.all(promesas);
  console.log("🧹 Todos los documentos fueron eliminados");
  alert("✅ Inventario de limpieza vaciado por completo");
  cargarDatos();
};
...
cargarDatos();
console.log("✅ script-limpieza.js cargado correctamente");
